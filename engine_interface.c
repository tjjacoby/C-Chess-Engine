...
int engine_makePlayerMove(uint64_t from, uint64_t to) {
    ...
    if (result == 1) {
        printf("Player move valid: piece at 0x%llx moved to 0x%llx\n", from, to);
        return 1;
    } else {
        printf("Player move invalid: piece at 0x%llx to 0x%llx (error code: %d)\n", from, to, result);
        return 0;
    }
}
...
SimpleMove engine_getAIMove() {
    ...
    if (aiMove.Piece != 0 || aiMove.Square != 0) {
        ...
        if (moveResult == 1) {
            printf("AI moved: piece at 0x%llx to 0x%llx\n", result.from, result.to);
        } else {
